...
<div class="space-y-6 max-w-4xl">
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Edit Izin Keluar</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Update data izin keluar siswa.</p>
        </div>
    </div>

    <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-8">
        <form action="{{ route('piket.exit_permissions.update', $exitPermission) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="student_id" id="student_id" class="mt-1 block w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all py-2.5 px-4" required>
                        ...
                        @foreach($students as $student)
                            <option value="{{ $student->id }}" {{ old('student_id', $exitPermission->student_id) == $student->id ? 'selected' : '' }}>{{ $student->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="teacher_id" id="teacher_id" class="mt-1 block w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all py-2.5 px-4" required>
                        ...
                        @foreach($teachers as $teacher)
                            <option value="{{ $teacher->id }}" {{ old('teacher_id', $exitPermission->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }} - {{ $teacher->subject ?? '-' }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <select name="piket_id" id="piket_id" class="mt-1 block w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all py-2.5 px-4" required>
                    ...
                    @foreach($pikets as $piket)
                        <option value="{{ $piket->id }}" {{ old('piket_id', $exitPermission->piket_id) == $piket->id ? 'selected' : '' }}>{{ $piket->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="reason" id="reason" rows="4" class="mt-1 block w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all py-2.5 px-4 resize-vertical" placeholder="Masukkan alasan keluar siswa" required>{{ old('reason', $exitPermission->reason) }}</textarea>
                <x-input-error :messages="$errors->get('reason')" class="mt-2" />
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <x-input-label for="leave_at" value="Waktu Keluar" />
                    <input type="time" name="leave_at" id="leave_at" value="{{ old('leave_at', $exitPermission->leave_at) }}" class="mt-1 block w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all py-2.5 px-4" required>
                    <x-input-error :messages="$errors->get('leave_at')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="return_at" value="Waktu Kembali (Opsional)" />
                    <input type="time" name="return_at" id="return_at" value="{{ old('return_at', $exitPermission->return_at) }}" class="mt-1 block w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all py-2.5 px-4">
            ...
            <div>
                ...
                <select name="status" id="status" class="mt-1 block w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all py-2.5 px-4" required>
                    <option value="pending" {{ old('status', $exitPermission->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                    <option value="approved" {{ old('status', $exitPermission->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                    <option value="completed" {{ old('status', $exitPermission->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                </select>
            ...
            <div class="flex flex-col sm:flex-row gap-3 pt-4">
                ...
                <x-primary-button type="submit" class="flex-1 sm:flex-none">Update Izin Keluar</x-primary-button>
            </div>
        </form>
    </div>
</div>
