...
<div class="space-y-8">
    <div>
        ...
        <p class="text-sm text-slate-500">Selamat datang kembali, {{ auth()->user()?->name ?? 'User' }}. Berikut ringkasan sistem hari ini.</p>
    </div>
        ...
        <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm transition-transform hover:scale-[1.02]">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-black text-slate-800 dark:text-white mt-1">{{ $stats['total_users'] ?? 0 }}</h3>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm transition-transform hover:scale-[1.02]">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-black text-slate-800 dark:text-white mt-1">{{ $stats['total_students'] ?? 0 }}</h3>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm transition-transform hover:scale-[1.02]">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-black text-slate-800 dark:text-white mt-1">{{ $stats['present_today'] ?? 0 }}</h3>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm transition-transform hover:scale-[1.02]">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-black text-slate-800 dark:text-white mt-1">{{ $stats['total_buildings'] ?? 0 }}</h3>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-1 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
            ...
            <div class="space-y-6">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <p class="text-sm font-bold text-slate-700 dark:text-slate-200">{{ optional($activity->student)->name ?? '—' }}</p>
                        <p class="text-xs text-slate-400">{{ optional(optional($activity->student)->classroom)->name ?? '—' }} • {{ optional($activity->created_at)->diffForHumans() ?? '-' }}</p>
                    </div>
                </div>
                @empty
                <p class="text-sm text-slate-400">Belum ada aktivitas kehadiran.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="lg:col-span-2 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
            ...
            <div class="h-64 flex items-end justify-between gap-2 px-4">
                @foreach($chartData ?? [] as $data)
                    @php
                        ...
                        $heightPct = $totalStudents ? (($data->aggregate ?? 0) / max($totalStudents, 1)) * 100 : 0;
                        $heightInt = (int) round(max(0, min(100, $heightPct)));
                    @endphp
                    <div class="flex-1 bg-blue-600/20 dark:bg-blue-400/10 rounded-t-lg relative group transition-all hover:bg-blue-600" 
                        style="<?php echo 'height: '.$heightInt.'%;' ?>">
            ...
            <div class="flex justify-between mt-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest px-2">
                @foreach($chartData ?? [] as $data)
                    <span>{{ date('d M', strtotime($data->date ?? now())) }}</span>
                @endforeach
            </div>
        </div>
    </div>
